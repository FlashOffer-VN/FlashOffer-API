...
│
├── docs/
│   └── FlashOffer-API.Documentation/       # Tài liệu dự án
│
├── src/
...
### Cấu hình .env (ưu tiên cao hơn appsettings.json)
...
**Quan trọng:** Thay đổi `Secret` trong `JwtSettings` hoặc `.env` thành khóa bí mật của riêng bạn (ít nhất 32 ký tự).
...
2. Cập nhật connection string trong `appsettings.json` hoặc `.env`
...
Đảm bảo `Secret` trong `appsettings.json` hoặc `.env` dài ít nhất 32 ký tự.
