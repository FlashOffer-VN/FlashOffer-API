...
- Layering: Domain models only in `FlashOffer-API.Domain`; DTOs in `Application/DTOs`.
- Controllers: inherit `ApiControllerBase`; use injected `IRepository<T>` / services.
- Exception handling: dùng `GlobalExceptionMiddleware` — không try/catch rộng trong controller.
...
- Comments: prefer self-explanatory code; chỉ comment cho intent hoặc complex logic.
